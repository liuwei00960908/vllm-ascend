...
class NPUModelRunner(GPUModelRunner):
    ...
    def _wait_for_bootstrap_kv_load(self) -> None:
        ...
        for latent_layer_name, indexer_layer_name in (
    ...
    def _bootstrap_load_failed_req_ids(
        ...
        if failed_req_ids:
            logger.warning(
                "KV load failed for final-hidden bootstrap request(s): %s.",
    ...
    def _execute_bootstrap_sample(
        ...
        if failed_req_ids:
            ...
            logger.warning(
                "Final-hidden bootstrap failed for request(s) %s; restarting "
                "them with dense prefill.",
                sorted(failed_req_ids),
            )
            return
    ...
    def _capture_final_hidden_producer_probe(
        ...
        *,
        ...
        """Save P-side snapshots before LM-head diagnostics synchronize them."""
        if (
            not envs_ascend.VLLM_ASCEND_FINAL_HIDDEN_PARITY_TRACE
            or not self._is_final_hidden_output_rank()
            or getattr(scheduler_output, "bootstrap_sample_req_ids", None)
        ):
            return

        captured_req_ids = getattr(
            scheduler_output,
        ...
        for req_idx, req_id in enumerate(self.input_batch.req_ids):
            if req_idx >= sample_hidden_states.shape[0]:
                break
    ...
    def _complete_final_hidden_producer_probe(
        ...
        """Add LM-head snapshots after hidden snapshots were frozen."""
        records = getattr(
            scheduler_output,
            "_final_hidden_producer_probe_records",
            None,
    ...
    def _finalize_final_hidden_producer_probe(
        ...
        """Log snapshots after sampling so the model path is not interrupted."""
        records = getattr(
            scheduler_output,
            "_final_hidden_producer_probe_records",
            None,
        ...
        for record in records:
            ...
            logger.warning(
                ...
                "scheduled_tokens=%d sample_row_index=%d "
                "num_tokens_unpadded=%d num_tokens_padded=%d "
                "hidden_shape=%s cudagraph_mode=%s hidden=%s logits=%s "
        ...
        logger.warning(
            ...
            "prompt_length_match=%s sample_row_match=%s hidden=%s logits=%s "
            "top1_match=%s sampled_token_match=%s",
            normal["request_id"],
