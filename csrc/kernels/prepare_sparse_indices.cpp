...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        // clear to finish before scalar updates start, otherwise the delayed
        // clear can erase request-union bits.
        AscendC::PipeBarrier<PIPE_V>();
        SyncPipeline<AscendC::HardEvent::V_S>();
        const uint64_t packedOffset =
            static_cast<uint64_t>(req) * scratchCapacity_;

        // One AIV owns one request, so bitmap updates need no atomics even
        // when multiple MTP rows select the same token position.
        for (uint32_t row = 0; row < rowCount_; ++row) {
            if (rowReqIndices_.GetValue(row) != static_cast<int32_t>(req)) {
                continue;
            }
        ...
        // in ascending absolute-token order.
...
extern "C" __global__ __aicore__ void dsa_prepare_sparse_indices_kernel(
    ...
    uint32_t clearInvalidRows)
{
    // Vector-only kernel. Mixed AIC/AIV launches execute the entry on both
    // core types; letting AIC perform the same in-place writes races with
