...
def main():
    ...
    row_req_indices = torch.full((args.rows,), -1, dtype=torch.int32, device=device)
    row_req_indices[: args.decode_rows] = torch.arange(args.decode_rows, dtype=torch.int32, device=device)
