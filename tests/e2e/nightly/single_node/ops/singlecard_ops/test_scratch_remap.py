...
def _reference(topk, split_boundary, valid_rows, scratch_base):
    return _scratch_remap_torch(
        topk,
        split_boundary,
...
def test_npu_dsa_scratch_remap_matches_torch_reference_exactly(rows, k, valid):
    enable_custom_op()
    try:
        fused_op = torch.ops._C_ascend.npu_dsa_scratch_remap_
    except AttributeError:
        pytest.skip("vllm_ascend_C was built without npu_dsa_scratch_remap_")
...
def test_npu_dsa_scratch_remap_typical_shape_performance():
    enable_custom_op()
    try:
        fused_op = torch.ops._C_ascend.npu_dsa_scratch_remap_
    except AttributeError:
    ...
    split_boundary_npu = torch.full((rows,), 131614, dtype=torch.int32).npu()
    valid_rows_npu = torch.arange(rows, dtype=torch.int32).npu()
    scratch_base_npu = (torch.arange(rows, dtype=torch.int32) * topk).npu()
    topk_npu = topk_cpu.npu()
...
def test_public_scratch_remap_dispatch_matches_torch_reference_exactly():
    enable_custom_op()
    if not hasattr(torch.ops._C_ascend, "npu_dsa_scratch_remap_"):
        pytest.skip("vllm_ascend_C was built without npu_dsa_scratch_remap_")
    ...
    actual_topk, actual_packed = scratch_remap(
        actual_input,
        split_boundary_npu,
...
def test_npu_dsa_scratch_remap_all_and_none_selected():
    enable_custom_op()
    try:
        fused_op = torch.ops._C_ascend.npu_dsa_scratch_remap_
    except AttributeError:
...
def test_npu_dsa_scratch_remap_without_packed_matches_torch_reference_exactly():
    enable_custom_op()
    try:
        fused_op = torch.ops._C_ascend.npu_dsa_scratch_remap_
    except AttributeError:
    ...
    valid_rows_npu = valid_rows_cpu.npu()
    scratch_base_npu = scratch_base_cpu.npu()
    expected_topk, expected_packed = _scratch_remap_torch(
        topk_npu.clone(),
        split_boundary_npu,
        need_packed=False,
        scratch_base=scratch_base_npu,
...
def test_npu_dsa_scratch_remap_large_int32_split_boundary_matches_exactly():
    enable_custom_op()
    try:
        fused_op = torch.ops._C_ascend.npu_dsa_scratch_remap_
    except AttributeError:
...
def test_npu_dsa_scratch_remap_empty_valid_rows_matches_torch_reference_exactly():
    enable_custom_op()
    try:
        fused_op = torch.ops._C_ascend.npu_dsa_scratch_remap_
    except AttributeError:
